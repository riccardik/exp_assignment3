...
<ul>
<li><code>SLEEP</code>, the robot is sleeping and so it wont respond to any command. After a while in the <code>NORMAL</code>, <code>FIND</code> or <code>PLAY</code> state, even if not commanded to do so, it reaches location []. After some amount of time, the robot goes again in state <code>NORMAL</code>. </li>
...
<p>
The package containing the assignment is <code>exp_assignment3</code>, in addition we have `move_base` and `explore_lite`.
